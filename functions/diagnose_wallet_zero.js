...
function calculateWalletBalances(profile, loans, payments, expenses, investments) {
  // Assume opening date is 0 (since starting of business)
  const openingDate = 0;
  const cashOpening = money(profile.cashOpeningBalance);
  const phoneOpening = money(profile.phonePeOpeningBalance);

  const walletLoans = loans;
  const walletPayments = payments;
  const walletExpenses = expenses;
  const walletInvestments = investments;
...
async function run() {
  ...
  console.log("\n=== Wallet Balances (Start of Business) ===");
